...
const defaultProps: Partial<HeroLandingProps> = {
  ...
  navigation: [
    { name: 'Practice Tracks', href: '/tracks' },
  ...
  callToActions: [
    { text: "Get started", href: "/auth/signup", variant: "primary" },
    ...
    if (cta.variant === 'primary') {
      return (
        <a
          key={index}
          href={cta.href}
          className="rounded-lg bg-white text-black px-3 py-2 sm:px-3.5 sm:py-2.5 text-xs sm:text-sm font-semibold shadow-sm hover:bg-gray-100 focus-visible:outline-2 focus-visible:outline-offset-2 transition-colors"
        >
          {cta.text}
        </a>
      )
      ...
      return (
        <a
          key={index}
          href={cta.href}
          className="text-xs sm:text-sm/6 font-semibold text-white hover:text-gray-300 transition-colors"
        >
      ...
      <header className="absolute inset-x-0 top-0 z-50">
        <nav aria-label="Global" className="flex items-center justify-between p-4 sm:p-6 lg:px-8">
          ...
          {navigation && navigation.length > 0 && (
            <div className="hidden lg:flex lg:gap-x-8 xl:gap-x-12">
              {navigation.map((item) => (
                <a key={item.name} href={item.href} className="text-sm/6 font-semibold text-gray-300 hover:text-white transition-colors">
                  {item.name}
                </a>
              ))}
            </div>
          ...
          {loginText && loginHref && (
            <div className="hidden lg:flex lg:flex-1 lg:justify-end">
              <a href={loginHref} className="text-sm/6 font-semibold text-gray-300 hover:text-white transition-colors">
        ...
        {mobileMenuOpen && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <div className="fixed inset-y-0 right-0 w-full overflow-y-auto bg-black/90 backdrop-blur-md px-4 py-4 sm:px-6 sm:py-6 sm:max-w-sm border-l border-gray-800">
                ...
                <div className="-my-6 divide-y divide-gray-800">
                  ...
                  {loginText && loginHref && (
                    <div className="py-6">
                      <a
                        href={loginHref}
                        className="-mx-3 block rounded-lg px-3 py-2.5 text-base/7 font-semibold text-white hover:bg-gray-800 transition-colors"
                      >
                        {loginText}
                      </a>
                    </div>
        ...
        <div className="mx-auto max-w-4xl pt-20 sm:pt-25">
          ...
          {announcementBanner && (
            <div className="hidden sm:mb-2 sm:flex sm:justify-center">
              <div className="relative rounded-full px-2 py-1 text-xs sm:px-3 sm:text-sm/6 text-gray-400 ring-1 ring-gray-700 hover:ring-gray-600 transition-all">
                {announcementBanner.text}{' '}
                <a href={announcementBanner.linkHref} className="font-semibold text-blue-400 hover:text-blue-300 transition-colors">
          ...
          <div className="text-center">
            ...
            <p className="mt-6 sm:mt-8 text-base sm:text-lg font-medium text-pretty text-gray-300 sm:text-xl/8">
              {description}
            </p>
